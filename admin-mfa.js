        ...
        setup.onclick = async () => {
            ...
            try {
                ...
                for (const factor of currentFactors.data?.all || []) {
                    if (factor.factor_type === "totp" && factor.status === "unverified" && factor.friendly_name === "Housing YQD administration") {
                        const removed = await client.auth.mfa.unenroll({ factorId: factor.id });
                        if (removed.error) throw removed.error;
                    }
                }
                const enrolled = await client.auth.mfa.enroll({ factorType: "totp", friendlyName: "Housing YQD administration", issuer: "Housing YQD" });
...
async function hyqdConfirmAdminServerProtection() {
    const { error } = await getHousingSupabaseClient().rpc("hyqd_record_admin_access");
